            ...
            try:
                with indexing_plugin__installed():
                    with self.logical_filesystem_for_indexing (objects, test_session):
                        ...
                        dummy_resultCount = func (search_index_for_avu_attribute_name(DEFAULT_METADATA_INDEX, 't-post-d1-attr'))
                        self.assertTrue( hit_source('object_path') == data_1 )
                        dummy_resultCount = func (search_index_for_avu_attribute_name(DEFAULT_METADATA_INDEX, 't-post-c1-attr')) # -- test [#42]
                        self.assertTrue( hit_source('object_path') == coll_1 )
                        dummy_resultCount = func (search_index_for_avu_attribute_name(DEFAULT_METADATA_INDEX, 't-post-c0-attr'))
            ...
            try:
                with indexing_plugin__installed():
                    with self.logical_filesystem_for_indexing (objects, test_session):
                        ...
                        dummy_resultCount = func (search_index_for_avu_attribute_name(DEFAULT_METADATA_INDEX, 't19-d1-attr'))
                        self.assertTrue( hit_source('object_path') == data_1)
                        dummy_resultCount = func (search_index_for_avu_attribute_name(DEFAULT_METADATA_INDEX, 't19-c1-attr'))
                        self.assertTrue( hit_source('object_path') == coll_1)
                        dummy_resultCount = func (search_index_for_avu_attribute_name(DEFAULT_METADATA_INDEX, 't19-c0-attr'))
                        self.assertTrue( hit_source('object_path') == test_path)
